...
static const std::string OUTPUT1 = "Output1"; //定义输出窗口名称
...
private:
    ...
    image_transport::Subscriber image_sub_1; //定义ROS图象接收器
...
public:
    RGB_GRAY()
      :it_(nh_) //构造函数
    {
        image_sub_ = it_.subscribe("camera/image/image_raw/left", 1, &RGB_GRAY::convert_callback, this); //定义图象接受器，订阅话题是“camera/rgb/image_raw”
        image_sub_1 = it_.subscribe("camera/image/image_raw/right", 1, &RGB_GRAY::convert_callback1, this); //定义图象接受器，订阅话题是“camera/rgb/image_raw”
    ...
    */
    ...
    {
       cv::Mat img_out;
       cv::cvtColor(img, img_out, CV_BGR2GRAY);  //转换成灰度图象
       cv::imshow(INPUT, img);
       cv::imshow(OUTPUT, img_out);
       cv::waitKey(5);
    }
    void image_process1(cv::Mat img) 
    {
       cv::Mat img_out;
       cv::cvtColor(img, img_out, CV_BGR2GRAY);  //转换成灰度图象
       cv::imshow(INPUT1, img);
       cv::imshow(OUTPUT1, img_out);
       cv::waitKey(5);
    }
